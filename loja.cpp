...
bool Produto::ler(istream& I)
{
  ...
  try
  {
    ...
    getline(I,nome,'"');
    if (I.fail() || nome=="") throw 1;
    // Ignora o ";" e o "$"
    I.ignore(numeric_limits<streamsize>::max(),'$');
    // Leh o preco como numero real
    I >> preco;
    if (I.fail() || preco<=0.0) throw 2;
  }
  catch (int i)
  {
    // Erro na leitura
